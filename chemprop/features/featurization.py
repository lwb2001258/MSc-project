    ...
    def __init__(self, mol: Union[str, Chem.Mol, Tuple[Chem.Mol, Chem.Mol]],
        ...
        if type(mol) == str:
            if self.is_reaction:
                ...
                # mol = make_mol(mol, self.is_explicit_h, self.is_adding_hs, True)
        ...
        self.edge_index = torch.tensor(list(bond_connect_info.values()), dtype=torch.long).T
        self.x = self.f_atoms
    ...
    * :code:`b2br`: (Optional): A mapping from f_bonds to real bonds in molecule recorded in targets.
    ...
    def __init__(self, mol_graphs: List[MolGraph]):
        ...
        for mol_graph in mol_graphs:
            ...
            mol_attrs.append(mol_graph.mol_attr)
    ...
    def get_components(self, atom_messages: bool = False) -> Tuple[torch.Tensor, torch.Tensor,
        ...
        * :code:`b_scope`
        ...
        return self.f_atoms, f_bonds, self.a2b, self.b2a, self.b2revb, self.a_scope, self.b_scope,self.mol_attrs
...
#     smiles = "CNC1CCCN(C1)c1c(F)cc2c(c1OC)n(cc(C(O)=O)c2=O)C1CC1"
#     mol = Chem.MolFromSmiles(smiles)
#     maccskeys = MACCSkeys.GenMACCSKeys(mol)
#     print(maccskeys)
#     print(maccskeys.ToBitString())
#     fp_list = list(maccskeys.ToBitString())
#     print(len(fp_list))
#     fp_list = list(map(lambda x: int(x), fp_list))
#     fp_tensor = torch.tensor(fp_list, dtype=torch.long)
#     print(len(fp_tensor))
