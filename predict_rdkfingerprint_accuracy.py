...
check_dirs = []
rootdir = 'predict/classification/RDKFINGERPRINT/'
for file in os.listdir(rootdir):
    d = os.path.join(rootdir, file)
    if os.path.isfile(d):
        check_dirs.append(d)
params_file = open("result/params_result.csv",'w')
...
for dir in check_dirs:
    try:
        ...
        with open("predict/classification/RDKFINGERPRINT/{}".format(file_name))as f:
            content = f.readlines()
        predict_result = {}
        for index,line in enumerate(content):
            if index != 0:
        ...
        try:
            ...
            json_file = open('test_models/classification/mol_attribute/RDKFINGERPRINT/{}/args.json'.format(dir_name), 'r')
            ...
            csv_file ="predict/classification/RDKFINGERPRINT/{}".format(file_name)
